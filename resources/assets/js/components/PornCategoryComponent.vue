        ...
        <div class="row">
            <div class="col-md-9">
        		<div class="row" v-for="i in Math.ceil(porns.length / 2)">
                    <div class="col-md-6" v-for="item in porns.slice((i - 1) * 2, i * 2)" @click.prevent="getPorn(item)">  
                        <div class="panel">
                            <div class="panel-heading">
...
<script>
    ...
    export default {
    	...
    	props: ['category_list', 'category_id'],
    	filters: {
    	...
    	methods: {
            getPorn(item) {
                window.location.href = "/porn/video/" + item.id;
            },
            getResults(page) {
                let vm = this;
                vm.loading = true;
                ...
                axios.get('/api/porn/category/' + this.categoryId + '?page=' + page)
                .then(function (response) {
                    vm.loading = false;
                    vm.porns = response.data.data;
                    console.log(vm.porns)
